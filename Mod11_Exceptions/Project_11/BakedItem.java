...
// Comparable interface added for project 10
public abstract class BakedItem implements Comparable<BakedItem> {
   ...
   */
   ...
   /**
   * compareTo accepts a BakedItem and returns an int indicating the ordering
   * from lowest to highest based on the class name followed name, and flavor.
   * 
   * @param bakedItemIn The BakedItem object that is being compared.
   * @return The int representing the result of the two variables being
   * compared.
   */
   public int compareTo(BakedItem bakedItemIn) {
   
      return this.toString().toLowerCase().compareTo(bakedItemIn.toString()
         .toLowerCase());
   }
}